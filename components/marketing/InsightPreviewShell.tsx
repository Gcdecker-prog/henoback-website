...
/** Outcome-first preview chrome — stats runway, minimal ERP chrome */
export function InsightPreviewShell({
  ...
  return (
    <div className={cn(dashboardCard, className)} role="region" aria-label={ariaLabel}>
      <div className="px-5 py-5 sm:px-6 sm:pt-6">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            ...
            <h3 className="mt-2 text-base font-semibold leading-snug tracking-tight text-neutral-900 sm:text-lg">
              {title}
            </h3>
            {subtitle ? (
        ...
        {stats && stats.length > 0 ? (
          <ul className="mt-6 grid grid-cols-3 gap-4 sm:gap-0 sm:divide-x sm:divide-neutral-200/90">
            {stats.map((stat) => (
              <li key={stat.label} className="min-w-0 sm:px-4 sm:first:pl-0 sm:last:pr-0">
                ...
                <p
                  className={cn(
                    ...
                    stat.tone === 'accent' && 'text-heno-blue-900',
                    stat.tone === 'positive' && brandUi.outcomeText,
                    !stat.tone || stat.tone === 'default' ? 'text-neutral-900' : '',
                  )}
                >
                  {stat.value}
                </p>
      ...
      <div className="border-t border-neutral-100/80 px-5 pb-5 pt-4 sm:px-6 sm:pb-6">
        {children}
